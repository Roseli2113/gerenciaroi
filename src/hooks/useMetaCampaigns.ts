...
function parseMetrics(insight: CampaignInsight | null) {
  ...
  const video3sec = insight.actions?.find(a => a.action_type === 'video_view')?.value;
    ...
    try {
      ...
      await Promise.all(activeAccountIds.map(async (accountId) => {
        ...
        const { data, error } = await supabase.functions.invoke('meta-ads', {
          body: {
            action: 'get-adsets-with-insights',
            accessToken,
        ...
        if (data?.insights) {
          data.insights.forEach((i: { adset_id: string } & Omit<CampaignInsight, 'campaign_id'>) =>
